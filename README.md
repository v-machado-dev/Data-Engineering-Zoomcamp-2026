
My learning journey through DataTalks.Club's  9-week course on 
building production-ready data pipelines.

![Status](https://img.shields.io/badge/Status-In%20Progress-yellow)
![Course](https://img.shields.io/badge/DataTalks.Club-DE%20Zoomcamp%202026-blue)
📎 [Course repository](https://github.com/DataTalksClub/data-engineering-zoomcamp)

## Modules

| # | Module | Tools | Status |
|---|--------|-------|--------|
| 1 | Containerization & Infrastructure as Code | Docker, Terraform, GCP | ⏳ |
| 2 | Workflow Orchestration | Kestra | ⏳ |
| - | Workshop: Data Ingestion | dlt | ⏳ |
| 3 | Data Warehousing | BigQuery | ⏳ |
| 4 | Analytics Engineering | dbt, DuckDB | ⏳ |
| 5 | Data Platforms | Bruin | ⏳ |
| 6 | Batch Processing | Apache Spark | ⏳ |
| 7 | Streaming | Apache Kafka | ⏳ |
| - | Final Project | — | ⏳ |

## Tech Stack

Docker · Terraform · GCP · Kestra · dlt · BigQuery · dbt · 
DuckDB · Bruin · Apache Spark · Apache Kafka

## Final Project

> Section to be updated upon course completion.

## Certificate

> To be added upon completion.
